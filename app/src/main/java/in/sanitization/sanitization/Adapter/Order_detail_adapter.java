...
import static in.sanitization.sanitization.Config.BaseUrl.IMG_PRODUCT_URL;
...
public class Order_detail_adapter extends RecyclerView.Adapter<Order_detail_adapter.MyViewHolder> {
  ...
  public Order_detail_adapter(List<My_order_detail_model> modelList) {
    this.modelList = modelList;
  }

  @Override
  public Order_detail_adapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    View itemView = LayoutInflater.from(parent.getContext())
            .inflate(R.layout.row_my_order_detail_rv, parent, false);

    context = parent.getContext();

    return new Order_detail_adapter.MyViewHolder(itemView);
  }

  @Override
  public void onBindViewHolder(Order_detail_adapter.MyViewHolder holder, int position) {
    ...
    try {
      ...
      if (mList.getProduct_image().equals(null)) {
        Glide.with(context)
                .load(IMG_PRODUCT_URL + mList.getProduct_image())
                .centerCrop()
